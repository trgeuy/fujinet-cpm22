...
# Function to generate an esptool write-flash argument list
def generate_esptool_command(json_file, usb_port):
    ...
    # sys.executable -m esptool: uses whichever esptool is pip-installed for
    # this interpreter (arch-independent, pure Python) instead of shelling out
    # to PlatformIO's bundled copy, which historically wasn't published for
    # macOS arm64 (Apple Silicon) and left this script Intel-Mac-only.
    # Both files go in one write-flash call so there's a single connect/reset
    # handshake with the chip instead of two.
    return [sys.executable, "-m", "esptool",
            "--port", usb_port, "--baud", str(UPLOAD_BAUDRATE), "write-flash",
            firmware_offset, firmware_path,
            filesystem_offset, filesystem_path]

# Function to upload firmware using an esptool command
def upload_firmware(esptool_command):
    print(" ".join(esptool_command))
    result = subprocess.run(esptool_command)
...
def monitor_device(usb_port):
    ...
    # pyserial's own miniterm ships as an esptool dependency, so it's always
    # present and arch-independent -- no PlatformIO install required just to
    # watch the console after flashing.
    subprocess.run([sys.executable, "-m", "serial.tools.miniterm",
                     usb_port, str(UPLOAD_BAUDRATE)])
...
def main():
    ...
    esptool_command = generate_esptool_command(json_file, usb_port)
    upload_firmware(esptool_command)
